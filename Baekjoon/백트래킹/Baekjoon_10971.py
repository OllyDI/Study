...
2 초	256 MB	54349	20246	12058	34.046%
문제
...
def backtracking(start, depth, val):
    global ans

    if depth == n - 1 and arr[start][0]:
        ans = min(ans, val + arr[start][0])
        return

    for i in range(n):
        if not visited[i] and arr[start][i]:
            visited[i] = True
            backtracking(i, depth + 1, val + arr[start][i])
            visited[i] = False
