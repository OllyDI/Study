...
1 초 (추가 시간 없음)	256 MB	17976	4827	3869	28.843%
문제
...
s = input().rstrip()
q = int(input())
res = [[0 for _ in range(26)] for _ in range(len(s) + 1)]

for i in range(1, len(s) + 1):
    for j in range(26): res[i][j] = res[i - 1][j]
    res[i][ord(s[i - 1]) - 97] += 1

for i in range(q):
    a, l, r = input().rstrip().split()
    print(res[int(r) + 1][ord(a) - 97] - res[int(l)][ord(a) - 97])